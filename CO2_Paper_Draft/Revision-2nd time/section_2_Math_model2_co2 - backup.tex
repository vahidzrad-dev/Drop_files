...
\begin{equation}\label{Eq:Pi}
    ...
    \end{aligned}
\end{equation}
where $p$ denotes the \replaced{pore pressure}{poroelastic medium pressure}\todo{YS: Do people really say ``poroelastic medium pressure''? It sounds strange.\\Vahid: Yes. I read for example in Wheeler \emph{et al.}~\cite{Wheeler201469}. Now I replaced it by ``pore pressure''.}, $\alpha\in[0,1]$ is the Biot coefficient. {Constant} $g_c\in\mathbb{R}^+$ is the strain energy released per unit length of fracture extension. {The strain energy density $\psi_0[\bm{\varepsilon}(\bm{u})]$ is given by}
\begin{equation}\label{Eq:Strain_Energy}
    ...
    \end{aligned}
\end{equation}
with $\lambda$ and $\replaced{G}{\mu}$ Lam\'e constants such that \replaced{$G>0$}{$\mu>0$} and \replaced{$\lambda+2G>0$}{$\lambda+2\mu>0$}. The linearized strain tensor takes the form:
\begin{equation*}
    \begin{aligned}
        \bm{\varepsilon}(\bm{u}) := 
        \frac{1}{2}\left(\nabla\bm{u}+\nabla\bm{u}^T\right).
...
\begin{remark}[Strain energy degradation]
...
\begin{remark}[Irreversibility constraint]
...
\todo[inline]{Vahid: I read somewhere by Bourdin \emph{et al.} that it has been shown the residual stiffness parameter $k$ has no significant impact on the numerical solution\dots}
...
\end{remark}
\todo[inline]{
%Vahid: I added one more remark to explain `the choice of $\ell$'. Let me know if you find it a good idea to put these explanations as a new remark.\\
YS: Equation \eqref{Eq:choice_ell} or something similar was proposed in 2014 by Bourdin.\\
%Vahid: Now I changed the reference to both Borden \emph{et al.} \cite{Borden2012A}, and Bourdin \emph{et al.} \cite{Bourdin2014014301}. The former one is earlier, and it is the one offering Equation \eqref{Eq:choice_ell}. The one in Bourdin \emph{et al.}'s work is with different coefficients. There, $\ell=\frac{3Eg_c}{8{\sigma_{cr}}^2}$.\\
YS: I believe we should not stop here. We need to know what led to the two different expressions -- the different types of loading?\\Vahid: Now I find the formula by Bourdin \emph{et al.} \cite{Bourdin2014014301} suits more for our case, since it is for AT1 models in spite of the other work by Borden \emph{et al.} \cite{Borden2012A}. The details on how they end up with this formula is given there.\\
...
\begin{remark}[The choice of $\ell$]\label{Re:4}
...
\todo[inline]{YS: Maybe we don't need this subsubsection at all.}
...
\begin{equation}\label{Eq:Mass_Conserv}
    \begin{aligned}
        \partial_t\left(\phi\rho\right) = -\nabla\cdot\left(\rho\bm{q}\right)+Q_g.
    \end{aligned}
\end{equation}
Here $\phi$ denotes the porosity of the porous medium (the fraction of volume occupied by the fluid), $\rho$ the density of the fluid, $\bm{q}$ the Darcy velocity vector, and \replaced{$Q_g$}{$Q_g=\rho_{i}q_{i}$}\todo{YS: Is summation over $i$ assumed here?\\Mostafa: It is the case if there is more than one resource. I think we can omit this in our case then.} the fluid source\deleted{, where $\rho_{i}$ and $q_i$ are the density and volume of the injected fluid, respectively}. Note that $Q_g$ has the unit of volumetric flow rate per unit volume. Also, we assume the rock is saturated by the fluid so that the fluid content in rock per volume is expressed by $\phi\rho$.
...
{The} left hand side of \eqref{Eq:Mass_Conserv}, the {rate of} change of fluid content{,} can be written as:
\begin{equation}\label{Eq:diff_m}
    \begin{aligned}
        \partial_t\left(\phi\rho\right) = \rho \; \partial_t \phi +\phi \; \partial_t \rho  = \rho \;  \partial_t \varepsilon_v+\phi  \left[\frac{\partial  \rho}{\partial p}  \partial_t p\right],
    \end{aligned}
\end{equation}
...
YS: But the exact derivation would have a term with $\nabla k$, or equivalently, $k'(d)\nabla d$.\\ Vahid: You are right. I changed the equation accordingly.\\
Another point: What is the justification for us to neglect the quadratic term? Is it because $p$ is the fluctuation value above some constant reference value?\\Vahid: It seems to me we are not allowed to neglect neither of them. Hence I added them both to Eq. \eqref{Eq:Darcy_right}. We should discuss more\dots}

\todo[inline]{YS: Shall we multiply \eqref{Eq:Darcy_right} by $-1$?\\Vahid: Done.}
Also, with \eqref{Eq:Darcy_law}, the first term of the right hand side of \eqref{Eq:Mass_Conserv} is computed as:
...
\begin{equation}\label{Eq:Density_Pressure}
    ...
    \end{aligned}
\end{equation}
where $R$ is the universal gas constant, and \replaced{$\varphi^r_{\delta}$ is the derivative of the residual part of the full expression of Helmholtz energy $\varphi^r$ with respect to the reduced density $\delta$. IT is written as:}{$\varphi^r_{\delta}$ is the derivative of the left hand side of \eqref{Eq:Res_Helmholtz} with respect to the reduced density $\delta$. Therein, the density of CO$_2$ is related only to the residual part of full expression, denoted as $\varphi^r$:}
...
\begin{equation*}\label{Eq:Delta}
    ...
    \end{aligned}
\end{equation*}
In \eqref{Eq:Res_Helmholtz}, $\rho_c$ and $T_c$ are the critical density and temperature, respectively, and $\delta=\rho/\rho_c$ and $\tau=T/T_c$ are the reduced ones. \added{Interested readers are referred to \cite{span1996new} for more explanations on other components of \eqref{Eq:Res_Helmholtz}.}
...
\deleted{The CO$_2$ density and pressure are related by:}
\begin{equation*} %\label{Eq:Density_Pressure}
    \begin{aligned}
        p=\left(1+\delta\varphi^r_{\delta}\right)\rho RT,
    \end{aligned}
\end{equation*}
\deleted{where $R$ is the universal gas constant, and $\varphi^r_{\delta}$ is the derivative of the left-hand-side of \eqref{Eq:Res_Helmholtz} with respect to the reduced density $\delta$.}
...
By substituting \eqref{Eq:diff_m}, \eqref{Eq:Darcy_right}, and \eqref{Eq:Density_Pressure} in \eqref{Eq:Mass_Conserv}, the governing equation for CO$_2$ flow is written as follows: 
...
%=  RT \left(1+\delta \varphi^r_{\delta} \right)+  \rho RT\varphi^r_{\delta}  \dfrac{\partial \delta  }{\partial \rho}+ \rho RT\varphi^r_{\delta}  \dfrac{\partial \varphi^r_{\delta}  }{\partial \rho} 
= RT\left(1+2\delta \varphi^r_{\delta}+\delta^2 \varphi^r_{\delta \delta} \right).
\end{split}
\end{equation}
\replaced{Here, }{Note that $\varphi^r_{\delta}$ and} $\varphi^r_{\delta \delta}$ \replaced{is}{are} the \deleted{first and} second derivative\deleted{s} of $\varphi^r$ with respect to $\delta$\deleted{, respectively}.
...
\paragraph{Incompressible fluid flow} Here, it is also worth mentioning the case of incompressibility. For the incompressible fluid flow, the left-hand-side of \eqref{Eq:Mass_Conserv} reads \cite{detournay1995fundamentals}:
...
\todo[inline]{YS: Here we need to summarize a non-redundant collection of equations.}
