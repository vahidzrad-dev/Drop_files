...
\begin{figure}[htbp]
    ...
    \subfloat[]{\includegraphics[width=75mm]{omega_insitu_phasefield}\label{Fig:omega_phasefield}}
        \caption{\added{Schematic of (a) a sharp fracture and (b) the diffusive phase field profile for the same fracture.} \added{In both figures, a} square plate with a borehole placed inside is shown. The \emph{in situ} stress is applied on the external boundary, while the fluid is injected from the boundary of the borehole. \added{We will only use the diffusive phase field in the proposed method.}%The \added{sharp} fracture $\mathcal{C}$ \added{in (a)} is approximated by a regularized crack surface $\mathcal{C}_{\ell}(d)$ \added{in (b)} which is a functional of the crack phase field.
        }
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
\added{See \ref{sub:weak_porous} for more details.}
...
On the other hand, we correlate the permeability to the phase field value \added{as it is clear that when the crack starts propagating, the permeability of the solid matrix starts to increase, as the crack width increases. To incorporate this effect into our model, an exponential variation of the fluid permeability has been adopted from \cite{PILLAI201836, ZHU2013179}:}
\begin{equation}\label{Eq:k_0}
\begin{aligned}
k(d) = k_0\exp{\left({\alpha}_k d\right)},
\end{aligned}
\end{equation}
where $k_0$ is the permeability of the intact material, and ${\alpha}_k$ is a coefficient to indicate the effect of phase field evolution on the permeability. We take ${\alpha}_k=7.0$\added{ according to \cite{ZHU2013179}. In real applications, ${\alpha}_k$ can be obtained from experiments.} %\deleted{see \cite{PILLAI201836,ZHU2013179} for more description.} 
Figure \ref{Fig:Permeability_increments} illustrates the permeability increasing with respect to the evolution of the phase field variable. \added{As seen therein, the permeability starts from an initial value for a completely intact porous medium to a very big value of permeability in the fully broken area.}
